...
for(let item of dizi){
    let sart = dizi[item]>0 && dizi[item]%2==0;
    if(sart){
        toplam +=item;
        adet++;
    }
...
yeniDizi1.sort((a,b)=> b-a);//siralamayi terse ceviriyor
...
let ortalama = Math.round(toplam1/notlar1.length);//yuvarliyoruz
...
ort = (ort/NodeFilter.length).toFixed(2);
